...
# 读取NDVI数据
ndvi_data = pd.read_csv('Data/WA/mean.csv', header=0)
...
merged_data.plot(column='WA_mean', cmap=cmap, linewidth=0.3, ax=ax, edgecolor='white', norm=norm)
...
ax.scatter(mk3_data.geometry.centroid.x, mk3_data.geometry.centroid.y, color='black', s=2.1, marker='o', label='p < 0.05')
...
plt.tight_layout()
# 保存图像
plt.savefig('map_WA_ave.png', dpi=500, bbox_inches='tight')
